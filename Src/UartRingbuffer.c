...
void RB_Uart_sendstring(UART_HandleTypeDef *huart, ring_buffer *_tx_buffer, const char *s){
	while(*s) RB_Uart_write(huart, _tx_buffer, *s++);
}
...
void RB_UART_ISR(UART_HandleTypeDef *huart, ring_buffer *_rx_buffer, ring_buffer *_tx_buffer){
    ...
    if (((isrflags & USART_ISR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET)) //adapted, USART_ISR_RXNE_RXFNE is not defined
    {
    	 /******************
    ...
    if (((isrflags & USART_ISR_TXE) != RESET) && ((cr1its & USART_CR1_TXEIE) != RESET)) // adapted, USART_ISR_TXE_TXFNF is not defined
    {
