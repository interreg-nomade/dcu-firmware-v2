...
#if PRINTF_PROTEUSII_DRIVER
  sprintf(string, "%u [proteusII_driver] [BT_connect] BT_transmit_CMD_Bytes()\n",(unsigned int) HAL_GetTick());
...
void BT_transmit_CMD_Bytes(UART_HandleTypeDef *huart, uint8_t cmd, uint16_t len, uint8_t *data)
{
...
#if PRINTF_PROTEUSII_DRIVER
  sprintf(string, "%u [proteusII_driver] [BT_transmit_CMD_Bytes] BT transmit buffer: ",(unsigned int) HAL_GetTick());
  xQueueSend(pPrintQueue, string, 0);
  sprintf(string, "");
  char DUString[3];
  ...
  xQueueSend(pPrintQueue, string, 0);
#endif
  HAL_UART_Transmit(&huart4, (uint8_t *)tx_buffer, len+5, 25);
}
...
void BT_transmit_CMD_Byte(UART_HandleTypeDef *huart, uint8_t cmd, uint8_t data)
{
...
#if PRINTF_PROTEUSII_DRIVER
  sprintf(string, "%u [proteusII_driver] [BT_transmit_CMD_Byte] BT transmit buffer: ",(unsigned int) HAL_GetTick());
  xQueueSend(pPrintQueue, string, 0);
  sprintf(string, "");
  char DUString[3];
  for (int i = 0; i < 6; i++)
  ...
  HAL_UART_Transmit(&huart4, (uint8_t *)tx_buffer, 6, 25);
...
void BT_transmit_CMD(UART_HandleTypeDef *huart, uint8_t cmd)
{
  ...
  HAL_UART_Transmit(&huart4, (uint8_t *)tx_buffer, 5, 25);
}
